...
fetch(url)
    .then(data => data.json())
    .then(json => {
        codingCard[1].querySelector('.rank').textContent = json.ranking;
        codingCard[1].querySelector('.solved').textContent = json.total_problems_solved;
        codingCard[1].querySelector('.rating').textContent = json.reputation;
    })
...
fetch(url)
    ...
    .then(json => {
        console.log(json);
        codingCard[2].querySelector('.rating').textContent = json['max rating'];
        codingCard[2].querySelector('.rank').textContent = json['max rank'];
    })
